...
export default function App() {
  ...
  const getMatchedViewFromUrl = (pathname: string, hashStr: string): string | null => {
    ...
    if (path === '/reset-password' || hash === '#reset-password' || path === '/reset' || hash === '#reset') return 'reset-password';
  ...
  useEffect(() => {
    const checkAuthAndRoute = async () => {
      try {
        ...
        // If recovery token is in URL, route straight to reset-password
        if (urlParams.get('token') || urlParams.get('reset_token')) {
        ...
        if (json.success && json.data?.user) {
          ...
          setCurrentUser(null);
          const protectedRoles = ['student', 'institution', 'industry', 'admin'];
          const currentPath = window.location.pathname.replace(/^\//, '').toLowerCase();
          if (protectedRoles.includes(currentPath)) {
            setCurrentView('home');
          }
        }
      } catch (err) {
