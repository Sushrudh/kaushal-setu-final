...
export function resolveUserOrDemo(req: AuthenticatedRequest, fallbackRole?: 'student' | 'institution' | 'industry' | 'admin'): AuthUser | null {
  ...
  // CRITICAL: NEVER allow fallback for admin role. Admin access requires authenticated session.
  if (fallbackRole && fallbackRole !== 'admin' && !token) {
...
export class OtpService {
  public static generateOtp(target: string, purpose: string): { otpId: string; code: string } {
    ...
    db.prepare(`
      INSERT INTO common_otp_verifications (id, target, otp_code, purpose, expires_at)
      VALUES (?, ?, ?, ?, ?)
    `).run(id, target, code, purpose, expiresAt);
