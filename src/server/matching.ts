...
export class SkillMatchingEngine {
  ...
  public getStudentSkillAnalysis(userId: string) {
    ...
    const competencyCategories = [
      { category: 'Programming (Python/Java)', target: 90, current: studentSkillNames.includes('Python') ? 90 : 20 },
      { category: 'Database & SQL', target: 85, current: studentSkillNames.includes('SQL & Database Design') ? 85 : 15 },
      { category: 'Frontend (React.js)', target: 80, current: studentSkillNames.includes('React.js') ? 80 : 25 },
      { category: 'Backend (Node.js)', target: 80, current: studentSkillNames.includes('Node.js & Express') ? 78 : 30 },
      { category: 'DevOps & Version Control', target: 75, current: studentSkillNames.includes('Git & GitHub') ? 80 : 20 },
      { category: 'AI & Data Science', target: 70, current: studentSkillNames.includes('Machine Learning') ? 75 : 10 }
    ];
