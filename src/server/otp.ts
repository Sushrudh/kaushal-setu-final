...
export async function requestOtp(
  ...
  if (!transporter) {
    console.warn(`[OTP] Email service not configured. EMAIL_USER and EMAIL_PASS environment variables are required.`);
    return {
      success: false,
  ...
  try {
    // 1. Dispatch real email via Nodemailer
    ...
    // Insert hashed OTP record into database
    db.prepare(`
