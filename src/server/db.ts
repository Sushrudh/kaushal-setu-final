...
export function initDatabase() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS common_password_resets (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS idx_password_resets_token ON common_password_resets (token_hash, is_used);
    CREATE INDEX IF NOT EXISTS idx_users_github_id ON common_users (github_id);
  `);

  try {
    db.exec('ALTER TABLE common_users ADD COLUMN firebase_uid TEXT;');
  } catch (e) {
    // Column already exists
  }

  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_users_firebase_uid ON common_users (firebase_uid);
