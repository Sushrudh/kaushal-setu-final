...
export function initDatabase() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS common_support_tickets (
      id TEXT PRIMARY KEY,
      user_id TEXT NOT NULL REFERENCES common_users(id),
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS admin_audit_logs (
      id TEXT PRIMARY KEY,
      admin_id TEXT NOT NULL REFERENCES common_users(id),
...
function seedInitialData() {
  ...
  const insertSkill = db.prepare(`
    INSERT INTO common_skills (id, name, category, description, in_demand)
    VALUES (?, ?, ?, ?, 1)
  `);
  ...
  const insertCourse = db.prepare(`
    INSERT INTO common_courses (id, title, provider, description, category, skills_covered, target_skill_id, target_skill_name, duration_hours, total_lessons, difficulty, thumbnail_url, rating, enrolled_count)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertLec = db.prepare(`
    INSERT INTO common_course_lectures (id, course_id, lesson_number, title, resource_url, duration_mins)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  insertUser.run('usr_std_1', 'student@kaushalsetu.in', defaultPasswordHash, 'student', 'Aarav Sharma', '9876543210', 'Indian Institute of Technology Delhi', 'IITD-2023-CS042', 1);

  // Student 2 (Priya Verma)
  insertUser.run('usr_std_2', 'priya.verma@nitk.ac.in', defaultPasswordHash, 'student', 'Priya Verma', '9811223344', 'National Institute of Technology Karnataka', 'NITK-ECE-2022', 1);

  // Institution User (Prof. Ramanathan)
  insertUser.run('usr_inst_1', 'institution@kaushalsetu.in', defaultPasswordHash, 'institution', 'Prof. K. Ramanathan', '9443210987', 'Delhi Technological University (DTU)', 'DTU-REG-8821', 1);

  // Industry User (Tata Consultancy Services & Tech Mahindra Lead)
  insertUser.run('usr_ind_1', 'industry@kaushalsetu.in', defaultPasswordHash, 'industry', 'Vikramaditya Singhania', '9988776655', 'TCS Digital Talent Solutions', 'TCS-HR-IND-90', 1);

  // Second Industry User (Infosys Innovation Hub)
  insertUser.run('usr_ind_2', 'recruiter@infosys.com', defaultPasswordHash, 'industry', 'Ananya Deshmukh', '9765432100', 'Infosys Engineering Services', 'INFY-RECRUIT-44', 1);

  // 4. Seed Student Profile
  const insertStdProf = db.prepare(`
    INSERT INTO students_profiles (user_id, bio, apaar_id, roll_number, institution_name, degree_program, graduation_year, current_cgpa, state, digilocker_status, digilocker_id, digilocker_verified_at, academic_qualification_verified, academic_verification_status, profile_completion_pct)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  insertStdProf.run(
    'usr_std_1',
  ...
  const insertDoc = db.prepare(`
    INSERT INTO students_documents (id, user_id, document_type, title, issuing_authority, document_number, verification_source, verification_status, extracted_metadata)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertStdCourse = db.prepare(`
    INSERT INTO students_courses (id, user_id, course_id, progress_pct, lessons_completed, status, started_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertProject = db.prepare(`
    INSERT INTO students_projects (id, user_id, title, description, technologies, skills_demonstrated, category, github_url, live_url, project_type, duration, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertInstProf = db.prepare(`
    INSERT INTO institutions_profiles (user_id, institution_name, aishe_code, institution_type, state, city, naac_grade, aicte_approved, nep_aligned, total_students, verified_students, departments_count, active_fdps, website, contact_person, contact_email)
  ...
  const insertDept = db.prepare(`
    INSERT INTO institutions_departments (id, institution_user_id, name, head_of_department, student_count, average_skill_score, placement_rate_pct)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertProg = db.prepare(`
    INSERT INTO institutions_programs (id, institution_user_id, title, type, department, description, partner_industry, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertIndProf = db.prepare(`
    INSERT INTO industry_profiles (user_id, company_name, cin_number, industry_sector, company_size, headquarters, website, verification_status, active_postings, total_hires)
  ...
  const insertJob = db.prepare(`
    INSERT INTO industry_jobs (id, industry_user_id, company_name, title, job_type, experience, location, workplace_type, stipend_salary, description, required_skills, preferred_skills, openings, deadline, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertApp = db.prepare(`
    INSERT INTO students_applications (id, user_id, job_id, company_name, job_title, match_score, matched_skills, missing_skills, status, applied_date, next_action)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  insertApp.run(
    'app_1',
  ...
  const insertFaq = db.prepare(`
    INSERT INTO common_faqs (id, category, question, answer, icon, keywords, display_order)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  for (const f of faqs) {
