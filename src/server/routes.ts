...
apiRouter.post('/students/me/github/sync', requireAuth, requireRole('student'), async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (existing) {
    db.prepare(`
      ...
      SET username = ?, github_username = ?, public_repos = ?, total_stars = ?, stars = ?, total_contributions = ?, top_languages = ?, profile_url = ?, last_synced_at = ?
      WHERE user_id = ?
    `).run(username, username, publicRepos, totalStars, totalStars, totalContributions, JSON.stringify(topLanguages), profileUrl, now, userId);
  } else {
    db.prepare(`
      INSERT INTO students_github (user_id, username, github_username, public_repos, total_stars, stars, total_contributions, top_languages, profile_url, last_synced_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(userId, username, username, publicRepos, totalStars, totalStars, totalContributions, JSON.stringify(topLanguages), profileUrl, now);
  }
