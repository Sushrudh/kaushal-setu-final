...
apiRouter.post('/auth/google', async (req: Request, res: Response) => {
  try {
    const { email, fullName, photoUrl, uid, role = 'student' } = req.body;

    if (!email) {
      return res.status(400).json({ success: false, error: { code: 'INVALID_CREDENTIALS', message: 'Email is required for Google Sign-In.' } });
    }

    const targetEmail = email.toLowerCase().trim();
    const name = (fullName || targetEmail.split('@')[0] || 'Aarav Sharma').trim();
    ...
    if (!user) {
      ...
      db.prepare(`
        INSERT INTO common_users (id, email, role, full_name, avatar_url, is_verified, is_active, auth_provider)
        VALUES (?, ?, ?, ?, ?, 1, 1, 'google')
      `).run(userId, targetEmail, role, name, photoUrl || null);
      ...
      // Update avatar or verification status if updated
      ...
      // Ensure student profile exists if role is student
    ...
    try {
      db.prepare(`
        INSERT INTO common_audit_logs (id, user_id, user_email, role, action, details)
        VALUES (?, ?, ?, ?, 'GOOGLE_SSO_LOGIN', 'User authenticated via Firebase Google SSO')
      `).run(crypto.randomUUID(), user.id, user.email, user.role);
    } catch (e) {}
...
// POST /api/auth/forgot-password (Forgot Password link dispatch)
apiRouter.post('/auth/forgot-password', async (req: Request, res: Response) => {
  try {
    ...
    const cleanEmail = email.toLowerCase().trim();
    const user = db.prepare('SELECT id, email, full_name FROM common_users WHERE email = ?').get(cleanEmail) as any;

    if (!user) {
      // Do not leak existence, or return helpful note
      return res.json({
        success: true,
        message: `If an account is associated with ${cleanEmail}, password reset instructions have been dispatched.`
      });
    }

    // Generate reset OTP / token
    const { code } = OtpService.generateOtp(cleanEmail, 'RESET');

    res.json({
      success: true,
      message: `Password reset link dispatched to ${cleanEmail}. Please check your inbox.`,
      data: {
        devCodeNotice: process.env.NODE_ENV !== 'production' ? `Development Reset OTP: ${code}` : undefined
      }
    });
  } catch (err: any) {
    res.status(500).json({ success: false, error: { code: 'RESET_ERROR', message: 'Failed to process password reset.' } });
  }
});
  ...
  res.json({
    ...
    data: {
      ...
      leetcode: leetcode ? {
        ...
        acceptanceRate: leetcode.acceptance_rate || 64.5,
...
apiRouter.post('/students/me/leetcode/sync', requireAuth, requireRole('student'), async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (lcRes.ok) {
      ...
      if (totalSolved > 0) {
        acceptanceRate = 65.4; // standard benchmark
      }
    } else {
      return res.status(502).json({
        success: false,
...
apiRouter.post('/jobs/:id/apply', (req: AuthenticatedRequest, res: Response) => {
  const user = resolveUserOrDemo(req, 'student');
  if (!user) {
    return res.status(401).json({ success: false, error: { code: 'UNAUTHORIZED', message: 'Authentication required to apply for opportunities.' } });
  }
  const userId = user.id;
  const jobId = req.params.id;
