...
apiRouter.get('/students/me/external-profiles', requireAuth, requireRole('student'), (req: AuthenticatedRequest, res: Response) => {
  ...
  let parsedTopLanguages: string[] = [];
  if (github?.top_languages) {
    try {
      parsedTopLanguages = JSON.parse(github.top_languages);
    } catch {
      parsedTopLanguages = typeof github.top_languages === 'string' ? github.top_languages.split(',').map((s: string) => s.trim()).filter(Boolean) : [];
    }
  } else if (github?.languages) {
    parsedTopLanguages = github.languages.split(',').map((s: string) => s.trim()).filter(Boolean);
  }
  ...
  res.json({
    ...
    data: {
      github: github ? {
        ...
        topLanguages: parsedTopLanguages,
