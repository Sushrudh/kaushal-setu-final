  ...
  const fetchStudentData = async () => {
    try {
      ...
      const [pData, jData, cData, aData] = await Promise.all([
        profileRes.ok ? profileRes.json().catch(() => ({})) : ({} as any),
        jobsRes.ok ? jobsRes.json().catch(() => ({})) : ({} as any),
        coursesRes.ok ? coursesRes.json().catch(() => ({})) : ({} as any),
        appsRes.ok ? appsRes.json().catch(() => ({})) : ({} as any)
      ]) as [any, any, any, any];
