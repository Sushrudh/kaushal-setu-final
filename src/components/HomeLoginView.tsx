  ...
  const handleAppleSSO = async () => {
    ...
    try {
      const res = await fetch('/api/auth/apple', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: 'student.apple@kaushalsetu.in', fullName: 'Aarav Sharma (Apple ID)', role: 'student' })
      });
      const json = await res.json();
      if (json.success) {
        onLoginSuccess(json.data.user, json.data.token);
      } else {
        setErrorMessage(json.error?.message || 'Apple Sign-In authorization failed.');
      }
    } catch (err) {
      setErrorMessage('Apple Sign-In service failed.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMicrosoftSSO = async () => {
    ...
    try {
      const res = await fetch('/api/auth/microsoft', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: 'student.msft@kaushalsetu.in', fullName: 'Aarav Sharma (Microsoft)', role: 'student' })
      });
      const json = await res.json();
      if (json.success) {
        onLoginSuccess(json.data.user, json.data.token);
      } else {
        setErrorMessage(json.error?.message || 'Microsoft SSO authorization failed.');
      }
    } catch (err) {
      setErrorMessage('Microsoft SSO service failed.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-8 sm:p-10 relative overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-2.5 mb-6">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border border-slate-200 bg-slate-50 hover:bg-white hover:border-blue-400 hover:shadow-md transition-all text-xs font-semibold text-slate-700 cursor-pointer"
                  title="Continue with Microsoft SSO"
                >
