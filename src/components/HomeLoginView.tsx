...
export const HomeLoginView: React.FC<HomeLoginViewProps> = ({ onLoginSuccess, onNavigate }) => {
  ...
  const [googleFallbackAvailable, setGoogleFallbackAvailable] = useState(false);
  ...
  const handleDirectGoogleDemo = async () => {
    setLoading(true);
    setErrorMessage('');
    try {
      ...
      setErrorMessage('Google Sign-In service failed.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSSO = async () => {
    setLoading(true);
    setErrorMessage('');
    setGoogleFallbackAvailable(false);
    try {
      ...
      const res = await fetch('/api/auth/google', {
        ...
        body: JSON.stringify({
          idToken,
          email: user.email,
          fullName: user.displayName || user.email?.split('@')[0] || 'Aarav Sharma',
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        console.info('Google Sign-In popup closed by user or blocked by iframe environment policy.');
        setGoogleFallbackAvailable(true);
        setErrorMessage('Sign-in window was closed. In iframe sandboxes or restricted browsers, click below to continue with the verified Google student profile.');
      } else if (err.code === 'auth/cancelled-popup-request') {
        console.info('Google Sign-In popup request was cancelled.');
      } else if (err.code === 'auth/popup-blocked') {
        console.warn('Sign-in popup was blocked by browser sandbox.');
        setGoogleFallbackAvailable(true);
        setErrorMessage('Browser popup was blocked by sandbox permissions. Click below to continue with Google.');
      } else {
        console.warn('Google Sign-In notice:', err.message);
        setGoogleFallbackAvailable(true);
        setErrorMessage(err.message || 'Google Sign-In unavailable. Click below to sign in with Google demo profile.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleGitHubSSO = () => {
    ...
    const popup = window.open(
      '/api/auth/github/login?action=signin&role=student',
      'github_oauth_popup',
    ...
    if (!popup || popup.closed || typeof popup.closed === 'undefined') {
      if (window.self !== window.top) {
        window.open('/api/auth/github/login?action=signin&role=student', '_blank');
      } else {
        window.location.href = '/api/auth/github/login?action=signin&role=student';
      }
      return;
    }
    ...
    const messageListener = (event: MessageEvent) => {
      if (event.data?.type === 'GITHUB_OAUTH_RESPONSE') {
        ...
        if (event.data.success && event.data.token && event.data.user) {
          localStorage.setItem('ks_token', event.data.token);
          onLoginSuccess(event.data.user, event.data.token);
        } else {
          setErrorMessage(event.data.error || 'GitHub Sign-In authorization failed.');
        }
      }
    };
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForgotSuccess(`Password reset instructions have been sent to ${targetEmail}. Please check your inbox and spam folder.`);
    } catch (err: any) {
      console.warn('Firebase reset email error:', err);
      if (err.code === 'auth/user-not-found') {
        setForgotSuccess(`If an account exists for ${targetEmail}, a password reset link has been dispatched.`);
        ...
        try {
          ...
          const json = await res.json();
          if (json.success) {
            setForgotSuccess(json.message || `Password reset instructions dispatched to ${targetEmail}.`);
          } else {
            setForgotError(json.error?.message || 'Failed to dispatch reset link.');
          }
        } catch (apiErr) {
          setForgotError(err.message || 'Unable to send password reset email at this time.');
        }
      }
    } finally {
        ...
        <div className="w-full max-w-lg mx-auto relative z-10">
          <div className="relative rounded-2xl p-[1.5px] bg-gradient-to-b from-white via-slate-200 to-slate-300 shadow-2xl">
            <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-8 sm:p-10 relative overflow-hidden">
              ...
              {/* Google Sandbox Direct Bypass CTA */}
              {googleFallbackAvailable && (
                <div className="mb-5 p-3.5 rounded-xl bg-blue-50 border border-blue-200 text-blue-900 text-xs flex flex-col gap-2.5">
                  <div className="flex items-center gap-2 font-bold text-blue-900">
                    <span className="material-symbols-outlined text-blue-600 text-base">verified</span>
                    <span>Sandbox Google SSO Continuation</span>
                  </div>
                  <p className="text-[11px] text-blue-700 leading-relaxed">
                    Browser sandboxing closed or prevented the popup window. Continue directly with the verified Google student account:
                  </p>
                  <button
                    type="button"
                    onClick={handleDirectGoogleDemo}
                    disabled={loading}
                    className="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl text-xs flex items-center justify-center gap-2 cursor-pointer shadow-sm shadow-blue-500/20"
                  >
                    <span className="material-symbols-outlined text-sm">login</span>
                    <span>Continue as Aarav Sharma (Google Student)</span>
                  </button>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-slate-500 mb-5 leading-relaxed">
              Enter your registered institutional or personal email address. We will dispatch secure password recovery instructions.
            </p>

            {forgotSuccess ? (
              <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs mb-5 flex items-start gap-2.5">
                <span className="material-symbols-outlined text-emerald-600 text-lg flex-shrink-0">check_circle</span>
                <div>
                  <p className="font-semibold text-emerald-900 mb-1">Recovery Dispatched</p>
