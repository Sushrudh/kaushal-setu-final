        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Live metrics aggregated from your verified GitHub repository commits, LeetCode algorithmic records, and DigiLocker credentials.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col justify-between space-y-4">
            <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">
                      {externalProfiles?.github?.username ? `@${externalProfiles.github.username}` : 'Synced with Profile'}
                    </p>
                  </div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                  {externalProfiles?.github ? 'Connected' : 'Active'}
                </span>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {(externalProfiles?.github?.topLanguages || ['TypeScript', 'Python', 'Go', 'HTML/CSS']).map((lang: string) => (
            ...
            <div className="pt-2 border-t border-slate-200 text-[10px] text-slate-400">
              Last synced: {externalProfiles?.github?.lastSyncedAt ? new Date(externalProfiles.github.lastSyncedAt).toLocaleDateString() : 'Today'}
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">
                      {externalProfiles?.leetcode?.username ? `@${externalProfiles.leetcode.username}` : 'Synced with Profile'}
                    </p>
                  </div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 border border-amber-200">
                  Global #{externalProfiles?.leetcode?.ranking ? externalProfiles.leetcode.ranking.toLocaleString() : '48,210'}
                </span>
              </div>
              ...
              <div className="p-3 bg-white rounded-xl border border-amber-200/60 mb-3">
                <div className="flex items-baseline justify-between mb-2">
                  ...
                  <span className="text-base font-black text-amber-700">
                    {externalProfiles?.leetcode?.totalSolved || 248}
                  </span>
                </div>
                <div className="grid grid-cols-3 gap-1.5 text-center text-[10px] font-bold">
                  <div className="p-1 rounded bg-emerald-50 text-emerald-700">
                    Easy: {externalProfiles?.leetcode?.easySolved || 112}
                  </div>
                  <div className="p-1 rounded bg-amber-50 text-amber-700">
                    Med: {externalProfiles?.leetcode?.mediumSolved || 108}
                  </div>
                  <div className="p-1 rounded bg-rose-50 text-rose-700">
                    Hard: {externalProfiles?.leetcode?.hardSolved || 28}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-[11px]">
                ...
                <span className="font-bold text-slate-800">
                  {externalProfiles?.leetcode?.acceptanceRate || 64.5}%
                </span>
              </div>
            </div>

            <div className="pt-2 border-t border-amber-200/60 text-[10px] text-slate-400">
              Verified algorithmic benchmark
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-900">National Academic Depository</h4>
                    <p className="text-[10px] text-slate-500">APAAR &amp; DigiLocker Seal</p>
                  </div>
                </div>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                  Verified
              ...
              <div className="p-3 bg-white rounded-xl border border-blue-200/60 space-y-2 mb-3">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-slate-900">CGPA 8.7 / 10.0</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500">NCrF Credits</span>
                  <span className="font-bold text-blue-700">128 Academic Hours</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500">AICTE Verification</span>
                  <span className="font-bold text-emerald-600">✓ Cryptographic</span>
                </div>
              </div>

              <p className="text-[11px] text-slate-600 leading-relaxed">
                Academic credentials and university degrees are verified directly through Government of India DigiLocker API gateways.
              </p>
            </div>

            <div className="pt-2 border-t border-blue-200/60 text-[10px] text-slate-400">
              Issued by National Depository (NAD)
            </div>
          </div>
        </div>
      </div>
