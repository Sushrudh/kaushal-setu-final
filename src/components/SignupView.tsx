...
export const SignupView: React.FC<SignupViewProps> = ({ onSignupSuccess, onNavigate }) => {
  ...
  const [pendingOtp, setPendingOtp] = useState<{ userId: string; token: string; email: string; otp?: string } | null>(null);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.data?.otpWarning) {
        setErrorMessage(`Account created, but email OTP delivery warning: ${json.data.otpWarning}`);
      }
      ...
      setPendingOtp({
        ...
        email: userEmail,
        otp: json.data?.otp || json.data?.devNotice,
      });
      setSuccessMessage('Registration initiated! A 6-digit verification code has been dispatched to your email.');
    } catch (err: any) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const userEmail = pendingOtp.email;
      ...
      setSuccessMessage('Email verified and registration completed successfully! Redirecting to sign in...');
      setTimeout(() => {
        onNavigate('home');
      }, 1800);
    } catch (err: any) {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json.success) {
        setOtpError(json.error?.message || 'Failed to resend verification code.');
      } else {
        const newOtp = json.otp || json.devNotice || json.data?.otp || json.data?.devNotice;
        if (newOtp) {
          setPendingOtp((prev) => prev ? { ...prev, otp: newOtp } : null);
        }
        setOtpCode('');
        setSuccessMessage('A fresh 6-digit OTP code has been dispatched to your email.');
      }
    } catch (err) {
      setOtpError('Error requesting new OTP.');
    } finally {
      setOtpLoading(false);
    }
  };
      ...
      if (res.ok && json.success) {
        try {
          await syncUserProfileToFirestore({
            ...
            role: json.data.user.role || role || 'student',
        ...
        onSignupSuccess(json.data.user, json.data.token);
      } else {
        setErrorMessage(json.error?.message || 'Google registration failed.');
      }
    } catch (err: any) {
      if (err.code === 'auth/popup-closed-by-user') {
        setErrorMessage('Google Sign-Up was cancelled.');
      } else if (err.code === 'auth/popup-blocked') {
        setErrorMessage('Sign-up popup was blocked by browser permissions. Please allow popups to continue with Google.');
      } else if (err.code === 'auth/operation-not-allowed') {
        setErrorMessage('Google provider is not enabled in Firebase Console. Please enable Google provider in Firebase.');
      } else {
  ...
  const handleGitHubSignup = async () => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        setErrorMessage(json.error?.message || 'GitHub registration failed.');
      }
    } catch (err: any) {
      if (err.code === 'auth/operation-not-allowed') {
        ...
        setErrorMessage('GitHub registration was cancelled.');
      } else if (err.code === 'auth/popup-blocked') {
        ...
        setErrorMessage(err.message || 'GitHub registration failed. Please try again.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <main className="relative -mt-32 max-w-3xl mx-auto px-4 sm:px-6 w-full pb-20 z-20">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200/80 overflow-hidden relative">
          ...
          <div className="p-6 sm:p-10">
            ...
            {/* Federated Quick Social Buttons - Google & GitHub ONLY */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              <button
                type="button"
                onClick={handleGoogleSignup}
                disabled={loading}
                className="inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold shadow-xs hover:shadow transition cursor-pointer disabled:opacity-50"
              >
              ...
              <div className="pt-2 space-y-2.5">
                <div className="flex items-start">
                  ...
                  <label className="ml-2.5 block text-xs text-slate-600 leading-normal" htmlFor="terms">
                    I agree to the Kaushal Setu{' '}
                    <button
                      type="button"
                      onClick={() => onNavigate('terms')}
                      className="text-blue-600 hover:underline font-medium inline p-0 bg-transparent border-none cursor-pointer"
                    >
                      Terms of Service
                    </button>
                    ,{' '}
                    ...
                    , and National Data Governance standards.
                  </label>
                </div>
      ...
      {pendingOtp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-slate-200">
            ...
            {pendingOtp.otp && (
              <div
                onClick={() => setOtpCode(pendingOtp.otp!)}
                className="mb-4 p-3 rounded-xl bg-blue-50 border border-blue-200 text-blue-900 text-xs flex items-center justify-between cursor-pointer hover:bg-blue-100/70 transition-colors"
                title="Click to fill OTP"
              >
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-base text-blue-600">verified_user</span>
                  <span className="font-semibold text-slate-700">Verification OTP:</span>
                </div>
                <span className="font-mono font-bold tracking-widest text-sm bg-white px-2.5 py-1 rounded-md border border-blue-200 text-blue-700 shadow-xs">
                  {pendingOtp.otp}
                </span>
              </div>
            )}
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className={`w-1/2 py-2.5 rounded-xl text-white text-sm font-semibold shadow-md ${
              ...
              <div className="pt-3 text-center border-t border-slate-100 mt-2">
                <p className="text-xs text-slate-500 mb-1">Didn't receive the verification code?</p>
                <button
                  type="button"
                  disabled={otpLoading}
                ...
                >
                  {otpLoading ? 'Sending new code...' : 'Resend OTP to email'}
                </button>
              </div>
            </form>
          </div>
