...
export const SignupView: React.FC<SignupViewProps> = ({ onSignupSuccess, onNavigate }) => {
  ...
  const [pendingOtp, setPendingOtp] = useState<{ userId: string; token: string; email: string } | null>(null);
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user' || err.code === 'auth/popup-blocked') {
        setErrorMessage('Google popup was closed or blocked. Please allow popups to continue.');
      } else {
  ...
  const handleGitHubSignup = () => {
    ...
    const popup = window.open(
      `/api/auth/github/login?action=signup&role=${encodeURIComponent(role)}`,
      'github_oauth_popup',
    ...
    if (!popup || popup.closed || typeof popup.closed === 'undefined') {
      if (window.self !== window.top) {
        window.open(`/api/auth/github/login?action=signup&role=${encodeURIComponent(role)}`, '_blank');
      } else {
        window.location.href = `/api/auth/github/login?action=signup&role=${encodeURIComponent(role)}`;
      }
      return;
    }
    ...
    const messageListener = (event: MessageEvent) => {
      if (event.data?.type === 'GITHUB_OAUTH_RESPONSE') {
        ...
        if (event.data.success && event.data.token && event.data.user) {
          ...
          onSignupSuccess(event.data.user, event.data.token);
        } else {
          setErrorMessage(event.data.error || 'GitHub Sign-Up authorization failed.');
        }
      }
    };
